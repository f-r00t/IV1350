...
public class InventoryHandler {
    ...
    public ItemDTO getItem(int identifier) throws ItemNotFoundException, DatabaseOfflineException {
        if(identifier == 404) {
            throw new DatabaseOfflineException(this.logger);
        }
        ItemDTO item = inventoryList.get(identifier);
        if (item == null) {
            throw new ItemNotFoundException(identifier, this.logger);
        }
        return item;
    }
